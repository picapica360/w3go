...
func NewFileAdapter(cfg string) func() zapcore.Core {
...
func NewFileAdapter2(opt Options) func() zapcore.Core {
	logger := lumberjack.Logger{}

	logger.Filename = opt.Filepath
	logger.MaxSize = opt.MaxSize
	logger.MaxAge = opt.MaxAge
	logger.MaxBackups = opt.MaxBackups
	logger.Compress = opt.Compress
...
func mustOtoa(obj interface{}) string {
	...
	default:
		panic(fmt.Errorf(`[logger] convert fail: can not convert "%v" to string`, obj))
	}
}
...
func mustOtoi(obj interface{}) (ret int) {
	...
	case int64:
		ret = int(v)
	default:
		panic(fmt.Errorf(`[logger] convert fail: can not convert "%v" to int`, obj))
	}
...
func mustOtob(obj interface{}) (ret bool) {
	...
	case bool:
		ret = v
	default:
		panic(fmt.Errorf(`[logger] convert fail: can not convert "%v" to bool`, obj))
	}
